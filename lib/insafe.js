...
function checkWhitelistedHostnames(url, whitelist) {
    ...
    return new Promise(function(resolve, reject) {
        if(whitelist.indexOf(host) != -1) {
            ...
            reject(host + ' is not whitelisted');
        }
    }).catch(function(err){
        throw err;
    });
...
exports.check = function(options) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        if(options.blacklist) {
            return checkBlacklistedHostnames(url, options.blacklist);
        } else {
            return true;
        }
    }).then(function() {
        if(options.whitelist) {
            return checkWhitelistedHostnames(url, options.whitelist);
        } else {
            return true;
        }
    }).catch(function(err) {
